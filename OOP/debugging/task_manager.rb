...
# The reasons the code continuted to return the nil class error was because the tasks variable, that was to be reassigned
# with the return value of the select method, had not been initialized and therefore returned nil. As ruby interpreted the 
# '=' as assignment, it attempted to iterates over the newly initialized local variable 'taks' which has no value. As a result,
# the select method cannot iterate over nil and return the error. THis can be avoided by making sure variable shadowing does not
# take place be providing unique names for the variables. 